...
void Mesh::CreateAllVertexBuffer()
{
	...
	allVertexBuffer = new GPUMemory(allVertices.size() * sizeof(Vertex), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, "VertexBuffer", true, allVertices.data());
}
...
void Mesh::CreateAllIndexBuffer()
{
	...
	allIndexBuffer = new GPUMemory(allIndices.size() * sizeof(uint32_t), VK_BUFFER_USAGE_INDEX_BUFFER_BIT, "IndexBuffer", true, allIndices.data());
}
...
static Mexel* LoadMexelFromFile(CHAR_T* filename)
{
	...
	{
		std::wcout << L"Mexel file: " << filename << L" does not exist!" << "\n";
		free(filename);
		return NULL;
	}
	...
	Mexel* mexel = LoadMexelFromBuffer(data.data(), NULL, (VulkanBackend*)GetEngine()->backend);
