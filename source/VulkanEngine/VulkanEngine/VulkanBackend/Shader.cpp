...
Shader::Shader(const CHAR_T* zlsl, const CHAR_T* vertfilename, const CHAR_T* pixlfilename, VkRenderPass renderPass, int shaderType, VkExtent2D screenSize, CullMode cullMode, PolygonMode polygonMode, VkSampleCountFlagBits sampleCount, BlendMode blendMode, bool depthTest, bool depthWrite, VkPushConstantRange* pushConstantRanges, uint32_t numPushConstantRanges, uint32_t stencilWriteMask, VkCompareOp stencilTestOp, uint32_t stencilTestValue, float depthBias, bool masked, VulkanBackend* backend)
{
...
#ifdef WIDE_STRINGS
Shader::Shader(const char* zlsl, const char* vertfilename, const char* pixlfilename, VkRenderPass renderPass, int shaderType, VkExtent2D screenSize, CullMode cullMode, PolygonMode polygonMode, VkSampleCountFlagBits sampleCount, BlendMode blendMode, bool depthTest, bool depthWrite, VkPushConstantRange* pushConstantRanges, uint32_t numPushConstantRanges, uint32_t stencilWriteMask, VkCompareOp stencilTestOp, uint32_t stencilTestValue, float depthBias, bool masked, VulkanBackend* backend)
{
	device = backend->logicalDevice;
...
struct SetLayoutNums
{
	uint32_t samplers, vbuffers, pbuffers, storage;
};

// if outVShader is NULL, then it's the same as the zlsl. It will need to be freed afterwards if valid.
template<typename T>
static std::vector<SetLayoutNums> GetInfoFromZLSL(const T* zlsl, uint32_t* outAttachments, bool vertexShaderOnly=false)
{
	std::vector<SetLayoutNums> setLayoutNums = { { 0, 0, 0, 0 } };
...
std::vector<VkDescriptorSetLayout> Shader::GetDescriptorSetLayoutsFromZLSL(const CHAR_T* filename, uint32_t* outAttachments)
{
	...
	for (const auto& nums : setLayoutNums)
		layouts.push_back(*DescriptorSet::GetDescriptorSetLayout(nums.vbuffers, nums.pbuffers, nums.samplers, nums.storage));
