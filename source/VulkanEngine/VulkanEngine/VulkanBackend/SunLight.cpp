...
SunLight::SunLight(float3 dir, uint32_t width, uint32_t height, Backend* backend)
{
	this->dir = dir;
	...
	for (uint32_t i = 0; i < NUMCASCADES; i++)
	{
		...
		vkCreateFramebuffer(vk->logicalDevice, &framebufferInfo, nullptr, &frameBuffers[i]);
	}

	viewProjBuffer.resize(vk->MAX_FRAMES_IN_FLIGHT);

	for (size_t i = 0; i < vk->MAX_FRAMES_IN_FLIGHT; i++)
		viewProjBuffer[i] = new GPUMemory((sizeof(float4x4) * NUMCASCADES) + sizeof(float4), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, "SunLight", false, NULL);

	descriptorSetVS.resize(vk->MAX_FRAMES_IN_FLIGHT);
	descriptorSetPS.resize(vk->MAX_FRAMES_IN_FLIGHT);
	...
	for (size_t j = 0; j < vk->MAX_FRAMES_IN_FLIGHT; j++)
	{
		offset = 0;
		for (uint32_t i = 0; i < NUMCASCADES; i++)
		{
		...
		vkUpdateDescriptorSets(vk->logicalDevice, NUMCASCADES, writes, 0, nullptr);
	}
	...
	for (size_t j = 0; j < vk->MAX_FRAMES_IN_FLIGHT; j++)
	{
		writes[0].dstSet = descriptorSetPS[j]->set;
		...
		vkUpdateDescriptorSets(vk->logicalDevice, NUMCASCADES + 1, writes, 0, nullptr);
	}
	...
	beginInfo.pClearValues = &vk->clearValues[1];
	...
	lightMapBeginInfo = &vk->beginInfo;
	sunOpaquePass = Light::renderPass;
}

SunLight::~SunLight()
{
	auto vk = (VulkanBackend*)GetEngine()->backend;
	...
	for (uint32_t i = 0; i < NUMCASCADES; i++)
	{
		vkDestroyFramebuffer(vk->logicalDevice, frameBuffers[i], VK_NULL_HANDLE);
