...
Texture::~Texture()
{
	...
	VkDevice device = ((VulkanBackend*)GetEngine()->backend)->logicalDevice;
...
{
	GPUMemory* stagingBuffer = new GPUMemory(sz, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, "Texture::CopyFromBuffer", false, NULL);
...
void Texture::CopyFromBuffer(GPUMemory* buffer) const
{
...
std::vector<float4> Texture::CopyToBuffer()
{
	...
	GPUMemory* imageBuffer = new GPUMemory(sizeof(float4) * size.x, VK_BUFFER_USAGE_TRANSFER_DST_BIT, "Copying image to buffer", false, NULL);
...
void Texture::CreateImageView(VkImageViewType viewType, VkImageViewCreateFlags flags)
{
	...
	if (vkCreateImageView(((VulkanBackend*)GetEngine()->backend)->logicalDevice, &viewInfo, nullptr, &view) != VK_SUCCESS)
