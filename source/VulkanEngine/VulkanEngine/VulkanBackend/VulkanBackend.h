...
#define BACKEND ((VulkanBackend*)GetEngine()->backend)
...
{
	zstring<CHAR_T>* text;
	GPUMemory* worldMatrix;
	GPUMemory* indexBuffer;
	GPUMemory* vertexBuffer;
	VkDescriptorSet descriptorSet;
...
class VulkanBackend : public Backend
{
	...
	std::vector<GPUMemory*> uniformBuffers;
	std::vector<GPUMemory*> psBuffers;
...
private:
	...
	void UpdateMaterialDescriptorSets(Material* mat) const;

public:
	VulkanBackend(GLFWwindow* glWindow, float resolutionScale);
	~VulkanBackend();
	...
	void AddLuaStuff(struct lua_State* L) override;
