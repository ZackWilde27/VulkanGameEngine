...
#define LuaBasicOperator(type, operation, pushfunc) 	auto v1 = LuaData<type>(L, 1); \
...
#define LuaOptionalNumberOperator(type, operation, pushfunc)	auto v1 = LuaData<type>(L, 1); \
\
																								auto v3 = (type*)lua_newuserdata(L, sizeof(type)); \
																								if (lua_isnumber(L, 2)) \
...
template<typename T>
T* LuaData(lua_State * L, int index)
{
	lua_getfield(L, index, LUA_DATA_NAME);
	auto data = (T*)lua_touserdata(L, -1);
	lua_pop(L, 1);
