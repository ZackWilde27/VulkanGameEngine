...
void main()
{
	...
	brightness *= 1-sqrt(sqrt(dst / pbo.lightPos.w));
	...
	float specularBrightness = sqrt(brightness);

	outColour.rgb = (d * brightness + (pow(saturate(-dot(reflection, dir)), coef) * (1-normal.a) * pbo.colour.rgb) * specularBrightness) * pbo.colour.rgb;
	outColour.a = 0;
}